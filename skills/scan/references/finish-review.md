...
2. Read `references/validator.md`, fill its slots (the same scope rules, requirements block, and review context the reviewers had, plus one block per finding in the shape that file shows), and dispatch one generic validator subagent. Retain the returned task ID and use the host's supported completion primitive to collect its result before continuing. Do not busy-poll or sleep. Peer findings are validated like every other finding; corroboration never skips the validator.
