...
1. **Settled elsewhere?** Re-read the prior-decisions block and any doc the candidate or its instances cite. A doc that settles the pattern the other way stops the fix; the candidate should have been dismissed, so record it in `reconciled.json` too.
...
After the edits, re-run the grep that found the instances and show the count is zero for the fixed scope. Then run the project's validation: the `Validation:` line in the `## Agent skills` block of `CLAUDE.md` or `AGENTS.md` when one exists, else the test suite, typecheck, and lint the project's conventions name. Target the touched area by default and broaden when the change spans packages.
