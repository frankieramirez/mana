...
Load at step 4b, after every fixer has returned and before the full validation run. One generic subagent, foreground, same model tier as the session. The orchestrator fills every `{slot}` and sends the result as the subagent's entire prompt. With a single item on the fix-list, skip the spawn and answer the three questions yourself on the diff.
