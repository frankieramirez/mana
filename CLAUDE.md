...
- Ticket files (`agent-brief.md`, `tickets.sh`) live under `skills/sift/` and are copied to `skills/conjure/`, `skills/cast/`, `skills/setup-mana/`, `skills/ultima/`, and (`tickets.sh` only) `skills/scan/` and `skills/attune/`. Edit the sift copies; the same script checks they match. `setup-mana` owns `references/triage-labels.md`, which `attune` copies under the same rule. Every label, issue, edge, claim, and `next` lookup on the tracker goes through `tickets.sh`, never a hand-written `gh issue` call.
