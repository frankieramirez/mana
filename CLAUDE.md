...
- `agents/comment-reaper.md` is generated. Edit `skills/decomment/references/comment-reaper.md` and run `scripts/sync-agent.sh`.
...
- `scripts/similarity.py` when touching `deep-review` or `address-feedback`: those started as forks and the rewrite is only done while every ratio stays under 0.30.
