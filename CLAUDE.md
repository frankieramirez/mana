...
- Everything under `agents/` is generated. Edit the reference file named in the table in `scripts/sync-agent.sh` (`skills/scrap/references/comment-reaper.md`, `skills/be-me/references/ghost.md`) and run that script. Adding an agent means a new table row plus a line in `plugin.json`.
...
- `scripts/similarity.py` when touching `wringer` or `settle`: those started as forks and the rewrite is only done while every ratio stays under 0.30.
