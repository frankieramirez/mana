...
- Ticket files (`agent-brief.md`, `tickets.sh`) live under `skills/sift/` and are copied to `skills/conjure/`, `skills/cast/`, `skills/setup-mana/`, and (`tickets.sh` only) `skills/scan/`. Edit the sift copies; the same script checks they match. Every label, issue, edge, claim, and `next` lookup on the tracker goes through `tickets.sh`, never a hand-written `gh issue` call.
